...
- (void) drawBackgroundForGlyphRange:(NSRange)glyphsToShow atPoint:(CGPoint)origin {
    ...
    [self enumerateLineFragmentsForGlyphRange:glyphsToShow
                                   usingBlock:^(CGRect rect, CGRect usedRect, NSTextContainer *textContainer, NSRange glyphRange, BOOL *stop) {
                                       ...
                                       if (charRange.location == paraRange.location) {
                                           gutterRect = CGRectOffset(CGRectMake(0, rect.origin.y, 30.0, rect.size.height), origin.x, origin.y);
                                           paraNumber = [self _paraNumberForRange:charRange];
                                           
                                           //AABB Pattern
                                           int mod = (paraNumber) % 4;
                                           NSString *paraString = @"";
                                           ...
                                           */
                                           ...
                                           //ABAB Pattern
                                           //NSString *paraString = (paraNumber + 1) % 2 == 0 ? @"B " : @"A ";
                                           
                                           NSString* ln = [NSString stringWithFormat:@"%@", paraString];
                                           //NSString* ln = [NSString stringWithFormat:@"%ld", paraNumber + 1];
    ...
    if (NSMaxRange(glyphsToShow) > self.numberOfGlyphs) {
        NSString *paraString = (paraNumber + 2) % 2 == 0 ? @"B " : @"A ";
        NSString* ln = [NSString stringWithFormat:@"%@", paraString];
